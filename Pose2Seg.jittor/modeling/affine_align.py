...
def affine_align_gpu(features, idxs, align_size, Hs):
    ...
    flow = nn.affine_grid(theta=Hs_var, size=(N, C_feat, H_feat, W_feat),align_corners=True).float()
    ...
    rois = nn.grid_sample(feature_select, flow, mode='bilinear', padding_mode='zeros',align_corners=True) # 'zeros' | 'border' 
    return rois
