...
def point_form(boxes):
    ...
    """
    return jt.contrib.concat((boxes[:, :2] - boxes[:, 2:]/2,     # xmin, ymin
...
def center_size(boxes):
    ...
    Return:
        boxes: (tensor) Converted xmin, ymin, xmax, ymax form of boxes.
    """
    return jt.contrib.concat(( (boxes[:, 2:] + boxes[:, :2])/2,     # cx, cy
...
def intersect(box_a, box_b):
    ...
    min_xy =  jt.maximum(box_a[:, :, :2].unsqueeze(2).expand((n, A, B, 2)),
                         box_b[:, :, :2].unsqueeze(1).expand((n, A, B, 2)))
    xy = jt.clamp(max_xy - min_xy, min_v=0)
    return xy[..., 0] * xy[..., 1]
...
def elemwise_mask_iou(masks_a, masks_b):
    ...
    return  jt.clamp(intersection /  jt.clamp(area_a + area_b - intersection, min_v=0.1), max_v=1)
