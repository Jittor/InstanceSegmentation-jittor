...
class MultiBoxLoss(nn.Module):
    ...
    def ohem_conf_loss(self, conf_data, conf_t, pos, num):
        ...
        conf_p = conf_data[(pos_idx.int()+neg_idx.int())>0].view(-1, self.num_classes)
        ...
        if cfg.use_class_balanced_conf:
            ...
            if self.class_instances is None:
                self.class_instances = jt.zeros(self.num_classes, device=targets_weighted.device)
    ...
    def focal_conf_sigmoid_loss(self, conf_data, conf_t):
        ...
        # From https://github.com/kuangliu/pyjt-retinanet/blob/master/utils.py
        conf_one_t = jt.eye(num_classes)[conf_t]
        conf_pm_t  = conf_one_t * 2 - 1 # -1 if background, +1 if forground for specific class

        logpt = nn.log_sigmoid(conf_data * conf_pm_t) # note: 1 - sigmoid(x) = sigmoid(-x)
        pt    = logpt.exp()
    ...
    def conf_objectness_loss(self, conf_data, conf_t, batch_size, loc_p, loc_t, priors):
        ...
        obj_pos_loss = - iou_targets * nn.log_sigmoid(obj_data_pos) - (1 - iou_targets) * nn.log_sigmoid(-obj_data_pos)
        ...
        for idx in range(mask_data.shape[0]):
            with jt.no_grad():
                ...
                for jdx in range(num_pos):
                    ...
                    new_mask = nn.AdaptiveAvgPool2d(cfg.mask_size)(tmp_mask.unsqueeze(0))
                    scaled_masks.append(new_mask.view(1, -1))

                mask_t = (jt.contrib.concat(scaled_masks, 0)>0.5).float() # Threshold downsampled mask
            
            pos_mask_data = mask_data[idx, cur_pos_idx_squeezed, :]
            loss_m += nn.bce_loss(jt.clamp(pos_mask_data, 0, 1), mask_t, size_average=False) * cfg.mask_alpha
    ...
    def coeff_diversity_loss(self, coeffs, instance_t):
        ...
        coeffs_norm = jt.normalize(coeffs, dim=1)
        cos_sim = coeffs_norm @ coeffs_norm.transpose(1,0)

        inst_eq = (instance_t.unsqueeze(1).expand_as(cos_sim) == instance_t.unsqueeze(0).expand_as(cos_sim)).float()
    ...
    def _mask_iou(self, mask1, mask2):
        intersection = jt.sum(mask1*mask2, dim=(0, 1))
        area1 = jt.sum(mask1, dim=(0, 1))
        area2 = jt.sum(mask2, dim=(0, 1))
    ...
    def mask_iou_loss(self, net, maskiou_targets):
        ...
        maskiou_p = jt.gather(maskiou_p, dim=1, index=label_t).view(-1)

        loss_i = nn.smooth_l1_loss(maskiou_p, maskiou_t, reduction='sum')
