...
def postprocess(det_output, w, h, batch_idx=0, interpolation_mode='bilinear',
    ...
    if dets is None:
        return [ jt.array([])] * 4 # Warning, this is 4 copies of the same thing
    ...
    if cfg.mask_type == mask_type.lincomb and cfg.eval_mask_branch:
        ...
        if cfg.use_maskiou:
            with timer.env('maskiou_net'):                
                with jt.no_grad():
                    ...
                    maskiou_p = jt.gather(maskiou_p, dim=1, index=classes.unsqueeze(1)).squeeze(1)
                    if cfg.rescore_mask:
                        if cfg.rescore_bbox:
        ...
        masks = nn.interpolate(masks.unsqueeze(0), (h, w), mode=interpolation_mode, align_corners=False).squeeze(0)
    ...
    for kdx in range(1):
        ...
        for y in range(arr_h):
            for x in range(arr_w):
                ...
                arr_img[y*proto_h:(y+1)*proto_h, x*proto_w:(x+1)*proto_w] = (proto_data[:, :, idx[i]] / jt.max(proto_data[:, :, idx[i]])).numpy() * coeffs_sort[i]
