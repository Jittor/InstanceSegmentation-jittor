...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, norm_layer=nn.BatchNorm, dilation=1, use_dcn=False):
        super(Bottleneck, self).__init__()
        self.conv1 = nn.Conv(inplanes, planes, kernel_size=1, bias=False, dilation=dilation)
        ...
        self.conv3 = nn.Conv(planes, planes * 4, kernel_size=1, bias=False, dilation=dilation)
...
class ResNetBackbone(nn.Module):
    ...
    def __init__(self, layers, dcn_layers=[0, 0, 0, 0], dcn_interval=1, atrous_layers=[], block=Bottleneck, norm_layer=nn.BatchNorm):
        super().__init__()
        ...
        self.backbone_modules = [m for m in self.modules() if isinstance(m, nn.Conv)]
...
class ResNetBackboneGN(ResNetBackbone):
    ...
    def init_backbone(self, path):
        ...
        for key in our_state_dict_keys:
            ...
            new_state_dict[key] = jt.array(state_dict[transcribed_key])
...
class DarkNetBackbone(nn.Module):
    ...
    def init_backbone(self, path):
        ...
        # Note: Using strict=False is berry scary. Triple check this.
        import torch
        self.load_parameters(torch.load(path))
...
class VGGBackbone(nn.Module):
    ...
    def __init__(self, cfg, extra_args=[], norm_layers=[]):
        ...
        self.norms = nn.ModuleList([nn.BatchNorm(self.channels[l]) for l in norm_layers])
    ...
    def add_layer(self, conv_channels=128, downsample=2):
        ...
        layer = nn.Sequential(
            nn.Conv(self.in_channels, conv_channels, kernel_size=1),
            nn.ReLU(),
            nn.Conv(conv_channels, conv_channels*2, kernel_size=3, stride=downsample, padding=padding),
            nn.ReLU()
        )
