...
class COCODetection(dataset.Dataset):
...
class EvalCOCODetection(dataset.Dataset):
    """`MS Coco Detection <http://mscoco.org/dataset/#detections-challenge2016>`_ Dataset.
    Args:
        root (string): Root directory where images are downloaded to.
        set_name (string): Name of the specific set of COCO images.
    ...
    def __init__(self, image_path, info_file, transform=None,
                 target_transform=None,
                 dataset_name='MS COCO', has_gt=True):
        super(EvalCOCODetection,self).__init__()
        # Do this here because we have too many things named COCO
        from pycocotools.coco import COCO
...
def enforce_size(img, targets, masks, num_crowds, new_w, new_h):
    ...
    with jt.no_grad():
        ...
        # Do all the resizing
        img = nn.interpolate(img.unsqueeze(0), (h_prime, w_prime), mode='bilinear', align_corners=False)
        img.squeeze_(0)

        # Act like each object is a color channel
        masks = nn.interpolate(masks.unsqueeze(0), (h_prime, w_prime), mode='bilinear', align_corners=False)
        masks.squeeze_(0)
