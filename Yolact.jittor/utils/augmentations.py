...
class ToTensor(object):
    def __call__(self, cvimage, masks=None, boxes=None, labels=None):
        return jt.array(cvimage.astype(np.float32)).permute(2, 0, 1), masks, boxes, labels
...
class FastBaseTransform(nn.Module):
    ...
    def __init__(self):
        ...
        self.mean = jt.array(list(MEANS)).float().unsqueeze(1).unsqueeze(2).unsqueeze(0)
        self.std  = jt.array( list(STD) ).float().unsqueeze(1).unsqueeze(2).unsqueeze(0)
