...
proto_masks = nn.interpolate(proto_masks, (h, w), mode='bilinear', align_corners=False).squeeze(0)
...
ious = mask_iou(jt.array(approximated_masks.reshape(-1, num_gt).transpose(1,0)),
				jt.array(gt_masks.reshape(-1, num_gt).transpose(1,0)))
