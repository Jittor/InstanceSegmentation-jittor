...
from detectron.modeling.roi_heads.box_head.inference import make_roi_box_post_processor
...
def im_detect_bbox_aug(model, images):
    ...
    boxlists_i = im_detect_bbox(
        model, images, cfg.INPUT.MIN_SIZE_TEST, cfg.INPUT.MAX_SIZE_TEST,
    )
...
def im_detect_bbox_scale(model, images, target_scale, target_max_size, hflip=False):
    """
    Computes bbox detections at the given scale.
    Returns predictions in the scaled image space.
    """
    if hflip:
        boxlists_scl = im_detect_bbox_hflip(model, images, target_scale, target_max_size)
    else:
        boxlists_scl = im_detect_bbox(model, images, target_scale, target_max_size)
    return boxlists_scl
