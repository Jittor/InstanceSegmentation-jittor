...
class ROIMaskHead(Module):
    ...
    def execute(self, features, proposals, targets=None):
        ...
        if self.is_training() and self.cfg.MODEL.ROI_MASK_HEAD.SHARE_BOX_FEATURE_EXTRACTOR:
            x = features
            x = x[jt.contrib.concat(positive_inds, dim=0)]
        else:
        ...
        if self.cfg.MODEL.MASKIOU_ON: 
            if not self.is_training():
                ...
                selected_mask = jt.contrib.concat(selected_mask,dim=0)
