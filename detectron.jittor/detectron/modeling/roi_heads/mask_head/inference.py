...
# object
class MaskPostProcessor(Module):
    """
...
class MaskPostProcessorCOCOFormat(MaskPostProcessor):
    ...
    def execute(self, x, boxes):
        ...
        results = super(MaskPostProcessorCOCOFormat, self).execute(x, boxes)
...
def paste_mask_in_image(mask, box, im_h, im_w, thresh=0.5, padding=1):
    ...
    box = expand_boxes(box.unsqueeze(0), scale)[0]
    box = box.int32().numpy()
    ...
    im_mask = jt.zeros((im_h, im_w)).bool()
    x_0 = max(box[0], 0)
