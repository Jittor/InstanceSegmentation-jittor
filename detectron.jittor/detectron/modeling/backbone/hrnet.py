...
class Bottleneck(Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv2 = nn.Conv(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = BatchNorm2d(planes, momentum=BN_MOMENTUM)
        self.conv3 = nn.Conv(planes, planes * self.expansion, kernel_size=1,
                               bias=False)
...
class HighResolutionNet(Module):
    ...
    def __init__(self,
        ...
        # stem network
        # stem net
        self.conv1 = nn.Conv(3, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
        self.bn1 = BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv(64, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1):
        ...
        if stride != 1 or inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv(inplanes, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
    ...
    def train(self,mode=True):
        if mode:
            ...
            super(HighResolutionNet, self).eval()
