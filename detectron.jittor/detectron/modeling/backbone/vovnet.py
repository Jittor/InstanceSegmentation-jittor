...
def conv3x3(in_channels, out_channels, module_name, postfix, stride=1, groups=1, kernel_size=3, padding=1):
    ...
    return [
        (f'{module_name}_{postfix}/conv',
         nn.Conv(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, groups=groups, bias=False)),
...
def conv1x1(in_channels, out_channels, module_name, postfix, stride=1, groups=1, kernel_size=1, padding=0):
    ...
    return [
        (f'{module_name}_{postfix}/conv',
         nn.Conv(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, groups=groups,
                   bias=False)),
        (f'{module_name}_{postfix}/norm',
            group_norm(out_channels) if _GN else FrozenBatchNorm2d(out_channels)
        ),
...
class eSEModule(Module):
    def __init__(self, channel, reduction=4):
        super(eSEModule, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Conv(channel,channel, kernel_size=1,
                             padding=0)
...
class _OSA_module(Module):
    ...
    def execute(self, x):
        ...
        x = jt.contrib.concat(output, dim=1)
        xt = self.concat(x)
...
class _OSA_stage(nn.Sequential):
    ...
    def __init__(self, 
        ...
        if not stage_num == 2:
            self.add_module('Pooling', nn.Pool(kernel_size=3, stride=2, ceil_mode=True,op='maximum'))
...
class VoVNet(Module):
    ...
    def __init__(self, cfg):
        ...
        for i in range(4):  # num_stages
            ...
            setattr(self,name, _OSA_stage(in_ch_list[i],
                                             config_stage_ch[i],
                                             config_concat_ch[i],
                                             block_per_stage[i],
                                             layer_per_block,
