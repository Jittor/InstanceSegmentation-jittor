...
from detectron.layers import mask_prob_cuda
import math
...
class EmbedMaskPostProcessor(nn.Module):
    ...
    def forward_for_single_feature_map(
        ...
        proposal_margin = proposal_margin.view(N, 1, H, W).transpose(0, 2, 3, 1)
    ...
    def forward_for_mask(self, boxlists, pixel_embed):
        ...
        for im in range(N):
            ...
            if self.post_process_masks:
                masks = nn.interpolate(X=masks.unsqueeze(1).float(), scale_factor=stride,
                                                        mode="bilinear", align_corners=False)> self.mask_th
    ...
    def execute(self, locations, box_cls, box_regression, centerness, proposal_embed, proposal_margin, pixel_embed, image_sizes, targets, benchmark, timers):
        """
        Arguments:
