...
class EmbedMaskHead(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        ########### Mask Predictions ############
        # proposal embedding
        self.proposal_spatial_embed_pred = nn.Conv(
            in_channels, 2, kernel_size=3, stride=1, padding=1, bias=True
        )
        self.proposal_other_embed_pred = nn.Conv(
        ...
        for modules in [self.proposal_spatial_embed_pred, self.proposal_other_embed_pred]:
            for l in modules.modules():
                if isinstance(l, nn.Conv):
                    ...
                    nn.init.constant_(l.bias, 0)
        # proposal margin
        self.proposal_margin_pred = nn.Conv(
        ...
        self.pixel_spatial_embed_pred = nn.Conv(
            in_channels, 2, kernel_size=3, stride=1, padding=1, bias=True
        )
        self.pixel_other_embed_pred = nn.Conv(
            in_channels, embed_dim-2, kernel_size=3, stride=1, padding=1, bias=True
        )
    ...
    def execute(self, x, locations, benchmark=False, timers=None):
        ...
        for l, feature in enumerate(x):
            ...
            coordinates = locations[l].transpose(1, 0).view(2, h, w).unsqueeze(0).expand((embed_x.shape[0], 2, h, w))
        ...
        coordinates = locations[0].transpose(1, 0).view(2, h, w).unsqueeze(0).expand((mask_x.shape[0], 2, h, w))
        scaled_coordinates = self.position_scale(coordinates) / 100.0
        pixel_spatial_embd = scaled_coordinates + pixel_spatial_embd
        pixel_embed = jt.contrib.concat([pixel_spatial_embd, pixel_other_embd], dim=1)
...
class EmbedMaskModule(nn.Module):
    """
    ...
    def execute(self, images, features, targets=None, benchmark=False, timers=None):
        """
        Arguments:
