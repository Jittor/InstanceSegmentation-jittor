...
from detectron.layers import LovaszHinge
from detectron.structures.boxlist_ops import boxlist_iou
from detectron.structures.boxlist_ops import boxlist_overlap
from detectron.structures.boxlist_ops import crop_by_box
from detectron.structures.boxlist_ops import boxes_to_masks
from detectron.layers.misc import interpolate
...
class EmbedMaskLossComputation(object):
    ...
    def prepare_targets(self, points, targets, im_w, im_h):
        ...
        for i in range(len(labels)):
            labels_split.append(jt.split(labels[i], num_points_per_level, dim=0))
            reg_targets_split.append(jt.split(reg_targets[i], num_points_per_level, dim=0))

        labels_level_first = []
        reg_targets_level_first = []
        for level in range(len(points)):
            labels_level_first.append(
                jt.contrib.concat([labels_per_im[level] for labels_per_im in labels_split], dim=0)
            )
    ...
    def get_sample_region(self, gt, strides, num_points_per, gt_xs, gt_ys, radius=1):
        ...
        for level, n_p in enumerate(num_points_per):
            ...
            center_gt[beg:end, :, 0] = jt.ternary(
                xmin > gt[beg:end, :, 0], xmin, gt[beg:end, :, 0]
            )
            center_gt[beg:end, :, 1] = jt.ternary(
                ymin > gt[beg:end, :, 1], ymin, gt[beg:end, :, 1]
            )
            center_gt[beg:end, :, 2] = jt.ternary(
                xmax > gt[beg:end, :, 2],
                gt[beg:end, :, 2], xmax
            )
            center_gt[beg:end, :, 3] = jt.ternary(
                ymax > gt[beg:end, :, 3],
                gt[beg:end, :, 3], ymax
            )
            beg = end
    ...
    def compute_mask_prob(self, proposal_embed, proposal_margin, pixel_embed):
        ...
        pixel_embed = pixel_embed.transpose(1, 2, 0).unsqueeze(0).expand(obj_num, -1, -1, -1)
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, proposal_embed, proposal_margin, pixel_embed, targets):
        ...
        for l in range(len(labels_per_level)):
            box_cls_flatten.append(box_cls[l].transpose(0, 2, 3, 1).reshape(-1, num_classes))
            box_regression_flatten.append(box_regression[l].transpose(0, 2, 3, 1).reshape(-1, 4))
