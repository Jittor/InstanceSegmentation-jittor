...
from detectron.modeling.rpn.retinanet.retinanet import build_retinanet
from detectron.modeling.rpn.fcos.fcos import build_fcos
from .embed_mask.embed_mask import build_embed_mask
...
class RPNHeadFeatureSingleConv(Module):
    ...
    def execute(self, x):
        assert isinstance(x, (list, tuple))
        x = [nn.relu(self.conv(z)) for z in x]
...
class RPNModule(Module):
    ...
    def _forward_test(self, anchors, objectness, rpn_box_regression):
        ...
        if self.cfg.MODEL.RPN_ONLY:
            ...
            inds = [
                box.get_field("objectness").argsort(descending=True)[0] for box in boxes
            ]
...
def build_rpn(cfg, in_channels):
    ...
    if cfg.MODEL.EMBED_MASK_ON:
        return build_embed_mask(cfg, in_channels)
