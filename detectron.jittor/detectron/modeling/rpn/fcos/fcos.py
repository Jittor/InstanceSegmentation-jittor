...
class FCOSModule(nn.Module):
    ...
    def compute_locations_per_level(self, h, w, stride):
        ...
        locations = self.get_dense_locations(locations, stride)
        return locations

    def get_dense_locations(self, locations, stride):
        ...
        if self.dense_points == 4:
            points = jt.array([l_t, r_t, l_b, r_b]).float()
        elif self.dense_points == 5:
            points = jt.array([l_t, r_t, [center, center], l_b, r_b]).float()
        else:
