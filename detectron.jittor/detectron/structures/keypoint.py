...
class Keypoints(object):
    ...
    def __repr__(self):
        ...
        s += 'num_instances={}, '.format(self.keypoints.shape[0])
...
def keypoints_to_heat_map(keypoints, rois, heatmap_size):
    ...
    scale_y = scale_y[:].unsqueeze(1)

    
    x = keypoints[..., 0]
    y = keypoints[..., 1]

    x_boundary_inds = x == rois[:, 2][:].unsqueeze(1)
    y_boundary_inds = y == rois[:, 3][:].unsqueeze(1)
